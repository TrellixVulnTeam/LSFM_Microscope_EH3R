#include "LSFM_Laser.h"
	...
	char* LSFM_Laser::SendCommand(char* SerialBuffer)
	{
	...
	void LSFM_Laser::ErrorHandling()
	{
		if (this->laserConnected == true)
		{
			...
			char* response = this->SendCommand(SerialBuffer);
			...
			response = this->SendCommand(SerialBuffer);

			string binaryValue = hex_str_to_bin_str(response);
	...
	bool LSFM_Laser::MoveRelativXYZ(int stage, double position)
	{
		// Ausnahme werfen
	...
	bool LSFM_Laser::MoveAbsolutXYZ(int stage, double position)
	{
	...
	int LSFM_Laser::ReadWaveLength()
	{
		if (this->laserConnected == true)
		{
			...
			int waveLength = extractValueFromResponse(response, "§");
	...
	float LSFM_Laser::ReadMaxPower()
	{
		if (this->laserConnected == true)
		{
			...
			float maxPower = extractValueFromResponse(response, "\r");
	...
	float LSFM_Laser::GetPowerInpercentage()
	{
		if (this->laserConnected == true)
		{
			...
			float intensityValue = extractValueFromResponse(response, "\r");
