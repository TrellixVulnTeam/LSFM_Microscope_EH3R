#include "LSFM_Laser.h"
	...
	float LSFM_Laser::extractValueFromResponse(char* response, int* specPower,  string delimiter)
	{
	...
	int LSFM_Laser::ReadWaveLength()
	{
		if (this->laserConnected == true)
		{
			...
			strcpy_s(SerialBuffer, command.c_str());
			 ...
			 int specPower = 0;
			int waveLength = extractValueFromResponse(SerialBuffer, &specPower, "§");
	...
	float LSFM_Laser::ReadMaxPower()
	{
		if (this->laserConnected == true)
		{
			...
			int specPower = 0;
			float maxPower = extractValueFromResponse(SerialBuffer, &specPower, "\r");
	...
	float LSFM_Laser::GetPowerInpercentage()
	{
		if (this->laserConnected == true)
		{
			...
			strcpy_s(SerialBuffer, command.c_str());
			 ...
			 int specPower = 0;
			float intensityValue = extractValueFromResponse(SerialBuffer, &specPower, "\r");
