...
vector<LSFM_Motion_PI> controllerDetection(char* description, int descriptionSize)
{
	...
	*/
	...
	const char* fileName = "C:\\Users\\User\\Documents\\TreiberImplementierung (2)\\TreiberImplementierung\\TreiberImplementierung\\ConfigFile.txt";
...
vector<LSFM_Laser> LaserPortDetection(char* filePath)
{
	vector<string> list1;
	vector<string>laserComList;
	vector<LSFM_Laser>laserList;
	string line;
	ifstream input;
	string delimiter = "->";
	size_t pos = 0;
	...
	if (input.is_open())
	{
		...
		for (int i = 0; i < list1.size(); i++)
		{
			...
			while ((pos = s.find(delimiter)) != std::string::npos) {
				...
				laserComList.push_back(s);
			}
		

			for (int i = 0; i < laserComList.size(); i++)
			{
				LSFM_Laser laser(laserComList.at(i));
