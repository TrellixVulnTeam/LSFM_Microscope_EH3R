...
void LSFM_Camera::SingleImageAquisition(int cameraTyp, HANDLE camera, int phaseNumber, int phasesymmetry, int tapSelect, char* file_name)
{
	...
	if (IsNoErrorOccured(iRet, "Eror during the Camera Busy Status acquisiton"))
	{
		...
		else
		{
			...
			if (IsNoErrorOccured(iRet, "Error during the Trigger Command"))
			{
				...
				GetAndSave_Image(cameratyp_i, camera, phaseNumber_i, PhaseSymmetry_i, tapSelect_i, file_name);
