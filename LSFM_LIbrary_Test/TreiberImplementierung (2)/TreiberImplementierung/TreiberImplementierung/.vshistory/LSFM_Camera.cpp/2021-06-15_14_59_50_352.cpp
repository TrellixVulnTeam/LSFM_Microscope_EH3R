...
bool LSFM_Camera::GetAndSave_Image(PhaseNumber phaseNumber, PhaseSymmetry phasesymmetry, TapSelect tapSelect, char* file_name)
{
            ...
            */
			...
			while (acquisitionRunning)
			{
				...
				if (IsNoErrorOccured(iRet, "Error with the Recorder Status"))
				{
					...
					// check if the camera has took L pictures
					if (procImgCount >= L)
					{
						acquisitionRunning = false;
						break;
		...
		else
		{
			...
			else if (this->GetCameraIdentification() == 2)
			{
				...
				if (IsNoErrorOccured(iRet, "Error with the Initialisation of the Recorder"))
				{
						...
						bool result = SetFlimPhaseSequenceParameter(PhaseNumber::Four, PhaseSymmetry::twice, PhaseOrder::opposite, TapSelect::both);
...
bool LSFM_Camera::Set_Camera_Master_modulation_frequency(DWORD frequency)
	{
		int iRet = PCO_SetFlimMasterModulationFrequency(this->cam, frequency);
...
bool LSFM_Camera::SingleImageAquisition(int phaseNumber, int phasesymmetry, int tapSelect, char* file_name)
{
		...
		if (IsNoErrorOccured(iRet, "Eror during the Camera Busy Status acquisiton"))
		{
			...
			else
			{
				...
				if (IsNoErrorOccured(iRet, "Error during the Trigger Command"))
				{
					...
					bool result = GetAndSave_Image(phaseNumber_i, PhaseSymmetry_i, tapSelect_i, file_name);
...
vector<WORD*> LSFM_Camera::Live_View(int* img_width, int* img_Height)
{
	vector<WORD*>bufferList;
	int cameraTyp = this->GetCameraIdentification();
					...
					if (cameraTyp == 1)
					{
						...
						while (acquisitionRunning)
						{
							...
							if (IsNoErrorOccured(iRet, "Error during the Live View"))
							{
								...
								iRet = PCO_RecorderCopyImage(this->hRec, hCamArr[0], PCO_RECORDER_LATEST_IMAGE, 1, 1, this->imgWidth, this->imgHeight, imgBuffer, NULL, &metadata, NULL);
					...
					else if (cameraTyp == 2)
					{
						...
						if (IsNoErrorOccured(iRet, "Error with the Acquisition of the flim Parameters"))
						{
							...
							for (int i = 0; i < procImgCount; i++)
							{
								...
								iRet = PCO_RecorderCopyImage(this->cam, hCamArr[0], i, 1, 1, imgWidth, imgHeight, imgBuffer, NULL, &metadata, NULL);
...
bool LSFM_Camera::SetFlimPhaseSequenceParameter(PhaseNumber phaseNumber, PhaseSymmetry phasesymmetry, PhaseOrder phaseOrder, TapSelect tapSelect)
{
	bool value = false;
	...
	else if (cameraType == 2)
	{
		...
		int iRet = PCO_SetFlimPhaseSequenceParameter(this->cam, phaseNumberparameter, phaseSymmetryparameter, phaseOrderparameter, tapSelectparameter, 0, 0);
