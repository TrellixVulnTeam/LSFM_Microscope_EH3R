...
vector<WORD*> LSFM_Camera::GetAndSave_Image(CameraTyp cameraTyp, HANDLE recorder, HANDLE camera, PhaseNumber phaseNumber, PhaseSymmetry phasesymmetry, TapSelect tapSelect, char* file_name)
{
...

	{
		...
		int L = 1;
		WORD phasenumber, phaseSymmetry, phaseOrder, tapSelect; 
		iRet = PCO_GetFlimPhaseSequenceParameter(camera, &phasenumber, &phaseSymmetry, &phaseOrder, &tapSelect, 0, 0);
		...
		if (IsNoErrorOccured(iRet, "Error with the Acquisition of the flim Parameters"))
		{
			...
			// Trigger Kommando senden
			WORD wTriggered;
			iRet = PCO_ForceTrigger(recorder, &wTriggered);

			if (IsNoErrorOccured(iRet, "Error  during Trigger Kommando"))

			{
				if (wTriggered)
				{
					cout << "A new  image exposure has been triggered" << endl;
				}
				else
				{
					cout << " trigger command was unsuccessful  because the camera is busy" << endl;
				}
			}
		...
		//Init Recorder
		reqImgCountArr[0] = L;
		printf("Number of required Images: %lu \n", reqImgCountArr[0]);
			...
			if (IsNoErrorOccured(iRet, "Error with the acquisition of Parameters"))
			{
				...
				*/
				...
				for (int i = 0; i < procImgCount; i++)
				{
					...
					iRet = PCO_RecorderCopyImage(recorder, hCamArr[0], i, 1, 1, imgWidth, imgHeight, imgBuffer, &imgNumber, &metadata, NULL);
