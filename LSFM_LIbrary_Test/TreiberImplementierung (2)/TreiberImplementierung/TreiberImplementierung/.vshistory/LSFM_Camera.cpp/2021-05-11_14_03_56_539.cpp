...

	{
		...
		if (IsNoErrorOccured(iRet, "Error with the initialisation of the Recorder"))
		{
			...
			if (IsNoErrorOccured(iRet, "Error with the acquisition of Parameters"))
			{
				...
				size_t size = ((__int64)imgWidth * (__int64)imgHeight) + size;
				size_t sizeTotal = 0;
				...
				for (int i = 0; i < procImgCount; i++)
				{
					...
					iRet = PCO_RecorderCopyImage(recorder, hCamArr[0], i, 1, 1, imgWidth, imgHeight, pBufferCurrent + sizeof(size), &imgNumber, &metadata, NULL);
