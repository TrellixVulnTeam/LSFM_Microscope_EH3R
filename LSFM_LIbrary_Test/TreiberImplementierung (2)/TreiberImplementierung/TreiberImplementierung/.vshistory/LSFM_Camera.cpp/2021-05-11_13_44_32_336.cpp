...

	{
		...
		if (IsNoErrorOccured(iRet, "Error with the initialisation of the Recorder"))
		{
			...
			if (IsNoErrorOccured(iRet, "Error with the acquisition of Parameters"))
			{
				...
				//datapack->bufferptr = NULL;

				WORD* pBuffer = NULL;
				WORD* pBufferCurrent = pBuffer;
				WORD sizetotal = 0;
				pBufferCurrent = new WORD[((__int64)imgWidth * (__int64)imgHeight)];
				...
				for (int i = 0; i < procImgCount; i++)
				{
					...
					//pBufferCurrent += (__int64)imgWidth * (__int64)imgHeight;

					iRet = PCO_RecorderCopyImage(recorder, hCamArr[0], i, 1, 1, imgWidth, imgHeight, pBufferCurrent, &procImgCount, &metadata, NULL);
...

					{
						...
						*/
						sizetotal += (__int64)imgWidth * (__int64)imgHeight;
