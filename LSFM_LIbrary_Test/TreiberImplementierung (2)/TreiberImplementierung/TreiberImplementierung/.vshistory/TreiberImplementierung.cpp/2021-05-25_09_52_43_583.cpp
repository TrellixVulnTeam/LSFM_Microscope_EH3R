...
void Live_View()
{
	...
	datapack = flimCameracontroller->Live_View(2, camHandle, camRecorderHandle);
...
int main()
{
	...
	WORD* imgBufferList = new WORD[bufferList.size() * ((__int64)1008 * (__int64)1008)];
	...
	datapack->width = 1008;
	datapack->buffersize = bufferList.size() * ((__int64)1008 * (__int64)1008);
	datapack->bufferptr = imgBufferList;


	 WORD* pBufferImage4 = BufferImageGetByIndex(imgBufferList, 3);


	char filenemame1[100];
	sprintf_s(filenemame1, "test1.tif");
	int iRet = PCO_RecorderSaveImage(pBufferImage4, 1008, 1008, FILESAVE_IMAGE_BW_16, false, filenemame1, true, nullptr);
