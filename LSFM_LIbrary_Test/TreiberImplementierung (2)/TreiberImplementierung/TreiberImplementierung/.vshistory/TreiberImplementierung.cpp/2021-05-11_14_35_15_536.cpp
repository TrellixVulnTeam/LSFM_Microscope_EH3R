...
int main()
{
	...
	//cameracontroller->SingleImageAquisition(2, camRecorderHandle, camHandle, NULL, NULL, NULL, filenemame);
	 DataPack* dataPack = flimCameracontroller->SingleImageAquisition(2, camRecorderHandle, camHandle, PhaseNumber::Four, PhaseSymmetry::twice, TapSelect::both, filenemame);

	 WORD* imgBuffer = NULL;

	 for (int i = 0; i < dataPack->imageCount; i++)
	 {
		 ...
		 imgBuffer +=  *((size_t*)dataPack->bufferptr);
		int  iRet = PCO_RecorderSaveImage(dataPack->bufferptr, dataPack->width, dataPack->height, FILESAVE_IMAGE_BW_16, false, filename, true, nullptr);
	 }
