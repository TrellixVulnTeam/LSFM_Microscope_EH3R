...
bool LSFM_FLIM_Camera::SetFilmPhaseSequenceParameter(PhaseNumber phaseNumber, PhaseSymmetry phasesymmetry, PhaseOrder phaseOrder, TapSelect tapSelect, DWORD frequency)
{
	...
	int iRet = PCO_RecorderStopRecord(this->flimHrec,this->flimCam);

	iRet = PCO_SetFlimPhaseSequenceParameter(this->flimCam, phaseNumberparameter, phaseSymmetryparameter, phaseOrderparameter, tapSelectparameter, 0, 0);
	...
	if (IsNoErrorOccured(iRet, "Problem with the Recording State\n"))
	{
		...
		if (IsNoErrorOccured(iRet, "Error during the Image processing Flow"))
		{
			...
			iRet = PCO_SetFlimMasterModulationFrequency(this->flimCam, frequency);

			if (IsNoErrorOccured(iRet, "Problem with the Modulation  Frequency\n"))
			{

				iRet = PCO_ArmCamera(this->flimCam);
				...
				if (IsNoErrorOccured(iRet, "Error with the Initialisation of Parameters"))
				{
					...
					iRet = PCO_GetFlimImageProcessingFlow(this->flimCam, &asymetricCorrection,&param1, &param2, &param3, &param4, &param5, &param6, &param7, &param8, &param9);
					...
					if (IsNoErrorOccured(iRet, "Error with the Acquisition of Asymmetry Correction "))
					{
						...
						iRet = PCO_GetFlimPhaseSequenceParameter(this->flimCam, &phaseNumberparameter, &phaseSymmetryparameter, &phaseOrderparameter, &tapSelectparameter, NULL, NULL);
						...
						if (IsNoErrorOccured(iRet, "Problem with the Recording State\n"))
						{
							...
							iRet = PCO_GetFlimMasterModulationFrequency(this->flimCam, &frequency);
...
{
	//int iRet = PCO_StopRecord(this->flimHrec, NULL, NULL);
	...
	if (IsNoErrorOccured(iRet, "Problem with the Modulation  Frequency\n"))
	{
		...
		//iRet = PCO_RecorderStartRecord(this->flimHrec, this->flimCam);
		...
		else
		{
			...
			if (IsNoErrorOccured(iRet, "Error with the Initialisation of the Recorder"))
			{
				...
				if (IsNoErrorOccured(iRet, "Error with the Start of  the Recorder"))
				{
					...
					bool result = SetFilmPhaseSequenceParameter(PhaseNumber::Four, PhaseSymmetry::twice, PhaseOrder::opposite, TapSelect::both, 30000000);
