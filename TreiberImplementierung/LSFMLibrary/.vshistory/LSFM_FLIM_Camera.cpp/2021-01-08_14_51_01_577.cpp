...
void LSFM_FLIM_Camera::Connect(){
	...
	while (isNoCameraConnected)
	{
		...
		else
		{
			...
			if (LSFM_FLIM_Camera::IsNoErrorOccured(iRet, "Error with the retribution of the Camera Setup"))
			{
				...
				else
				{
					...
					CheckAndSetTriggerMode(this->flimCam, 0);

					this->cameraMode = PCO_RECORDER_MEMORY_RINGBUF;

					Prepare_Recording(this->flimCam, this->flimHrec, PCO_RECORDER_MODE_MEMORY);

					//Init Recorder
					iRet = PCO_RecorderInit(this->flimHrec, reqImgCountArr, 1, this->cameraMode, 0, NULL, NULL);

					if (LSFM_FLIM_Camera::IsNoErrorOccured(iRet, "Error with the Initialisation of the Recorder"))
					{
