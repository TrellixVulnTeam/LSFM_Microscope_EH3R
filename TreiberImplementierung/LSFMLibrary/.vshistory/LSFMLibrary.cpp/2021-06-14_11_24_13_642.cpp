...
bool SetFlimParamter(int phaseNumber, int frequency)
{
	...
	pmicIm_2->SetFilmPhaseSequenceParameter(phaseNumber1, PhaseSymmetry::twice, PhaseOrder::opposite, TapSelect::both, frequency);
...
void Image_Sequence(int cameraType, const char* path, double zUp, double zDown, double xValue, double yValue, double focusUp, double focusDown, int N, double dz, int phaseNumber)
{
	...
	else
	{
		for (int i = 0; i < N; i++)
		{
			...
			PhaseNumber phasenumber1 = static_cast<PhaseNumber>(phaseNumber);
			pmicIm_2->SingleImageAquisition(2, flimCameraRecorderPointer, flimCameraPointer, phasenumber1, PhaseSymmetry::twice, TapSelect::both, TotalLine);
