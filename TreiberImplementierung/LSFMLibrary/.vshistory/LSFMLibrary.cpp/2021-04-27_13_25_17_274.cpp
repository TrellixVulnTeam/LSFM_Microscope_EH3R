...
void SingleImageAquisition(int cameratyp , char* filename)
{
	cout << filename << endl;
	if(cameratyp == 1)
	pmicIm_1->SingleImageAquisition(1, cameraRecorderPointer, cameraPointer, NULL, NULL, NULL, filename);
	else
		pmicIm_2->SingleImageAquisition(2, flimCameraPointer, flimCameraRecorderPointer, 4, 2, 0, filename);
