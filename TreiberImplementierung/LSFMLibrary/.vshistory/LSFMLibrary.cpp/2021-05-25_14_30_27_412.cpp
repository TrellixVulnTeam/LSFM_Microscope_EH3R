...
void SingleImageAquisition(int cameratyp , char* filename, DataPack* datapack1, DataPack* datapack2, DataPack* datapack3, DataPack* datapack4)
{
	cout << filename << endl;
	...
	if (cameratyp == 1)
	{
		vector<WORD*> bufferList = pmicIm_1->SingleImageAquisition(1, cameraRecorderPointer, cameraPointer, NULL, NULL, NULL, filename);

	}
	else
	{
		//vector<WORD*> bufferList = pmicIm_2->SingleImageAquisition(2, flimCameraRecorderPointer,flimCameraPointer, PhaseNumber::Four, PhaseSymmetry::twice, TapSelect::both, filename);

		vector<WORD*> bufferList = pmicIm_2->SingleImageAquisition(2, flimCameraRecorderPointer, flimCameraPointer, PhaseNumber::Four, PhaseSymmetry::twice, TapSelect::both, filename);


		//bufferContent = bufferList.data();
		
		
		datapack1->bufferptr = bufferList.at(0);
		datapack2->bufferptr = bufferList.at(1);
		datapack3->bufferptr = bufferList.at(2);
		datapack4->bufferptr = bufferList.at(3);
		

		//datapack->height = 1008;
		//datapack->width = 1008;
		//datapack->imgNumber = bufferList.size();
		//datapack->bufferptr = arr;
		//address = arr;
		...
		datapack->height = 1008;
		datapack->width = 1008;
		datapack->buffersize = bufferList.size() * sizeof (bufferList.at(0));
