...
void LaserOff(int wavelength, int specPower)
{
	...
	/*
	if (pmlaser->ReadWaveLength() == wavelength && pmlaser->ReadSpecPower() == specPower)
		pmlaser->LaserOff();
	else if (pmlaser2->ReadWaveLength() == wavelength && pmlaser2->ReadSpecPower() == specPower)
		pmlaser2->LaserOff();
	else if (pmlaser3->ReadWaveLength() == wavelength && pmlaser3->ReadSpecPower() == specPower)
		pmlaser3->LaserOff();
	else
...
void Disconnect()
{
	...
	/*
	microscope.remove(laserComponent1);
	microscope.remove(laserComponent2);
	microscope.remove(laserComponent3);
	*/
...
int GetActualModulation(int wavelength, int specPower)
{
	...
	if (pmlaser->ReadWaveLength() == wavelength && pmlaser->ReadSpecPower() == specPower)
		...
		actualModulation = pmlaser3->GetActualModulation();
	else
		cout << "The entry value has not been recognized, please enter a good value." << endl;
 ...
 bool GetWaveLengthAndSpecPower(Laserpack* LaserList)
{
	 ...
	 {
		 cout << "Test1" << endl;
		 cout << lasers.at(1)->ReadWaveLength() << endl;
		 cout << "Test2" << endl;
	 ...
	 /*
	Laserpack l1;
	if (pmlaser->IsHardwareConnected())
	{
		l1.waveLegnth = pmlaser->ReadWaveLength();
		l1.specPower = pmlaser->ReadSpecPower();
		LaserList[0] = l1;
	}
	...
	Laserpack l2;
	if (pmlaser2->IsHardwareConnected())
	{
		l2.waveLegnth = pmlaser2->ReadWaveLength();
		l2.specPower = pmlaser2->ReadSpecPower();
		LaserList[1] = l2;
	}
	...
	Laserpack l3;
	if (pmlaser3->IsHardwareConnected())
	{
		l3.waveLegnth = pmlaser3->ReadWaveLength();
		l3.specPower = pmlaser3->ReadSpecPower();
		LaserList[2] = l3;
	}
