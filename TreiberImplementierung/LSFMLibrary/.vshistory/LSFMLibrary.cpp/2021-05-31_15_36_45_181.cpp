...
void LaserOn(int wavelength)
{
	...
	{
		cout << "ajksfkafkkaskdsd" << endl;
		//cout << pmlaser1->ReadWaveLength() << endl;
		/*
...
DataPack_FLIM* SingleImageAquisition(int cameratyp, char* filename)
{
	cout << filename << endl;

	DataPack_FLIM* datapack = new DataPack_FLIM();
	int imgwidth, imgheight;

	if (cameratyp == 1)
	{
		bufferList = pmicIm_1->SingleImageAquisition(1, cameraRecorderPointer, cameraPointer, NULL, NULL, NULL, filename, &imgwidth, &imgheight);

	}
	else
	{

		bufferList = pmicIm_2->SingleImageAquisition(2, flimCameraRecorderPointer, flimCameraPointer, PhaseNumber::Four, PhaseSymmetry::twice, TapSelect::both, filename, &imgwidth, &imgheight);
		...
		if (bufferList.size() > 0)
		{
			...
			datapack->imgNumber = bufferList.size();

			switch (bufferList.size())
			{
			case 4:
				datapack->bufferptr1 = bufferList.at(0);
				datapack->bufferptr2 = bufferList.at(1);
				datapack->bufferptr3 = bufferList.at(2);
				datapack->bufferptr4 = bufferList.at(3);
			...
			case 8:
				...
				datapack->bufferptr3 = bufferList.at(2);
				datapack->bufferptr4 = bufferList.at(3);
				datapack->bufferptr5 = bufferList.at(4);
				datapack->bufferptr6 = bufferList.at(5);
				datapack->bufferptr7 = bufferList.at(6);
				datapack->bufferptr8 = bufferList.at(7);
				break;
			...
			case 16:
				...
				datapack->bufferptr7 = bufferList.at(6);
				datapack->bufferptr8 = bufferList.at(7);
				datapack->bufferptr9 = bufferList.at(8);
				datapack->bufferptr10 = bufferList.at(9);
				datapack->bufferptr11 = bufferList.at(10);
				datapack->bufferptr12 = bufferList.at(11);
				datapack->bufferptr13 = bufferList.at(12);
				datapack->bufferptr14 = bufferList.at(13);
				datapack->bufferptr15 = bufferList.at(14);
				datapack->bufferptr16 = bufferList.at(15);
				break;
...
void Image_Sequence(const char* path, double zUp, double zDown, double xValue, double yValue, double focusUp, double focusDown, int N, double dz)
{
	...
	for (int i = 0; i < N; i++)
	{
		...
		*/
		...
		pmicIm_1->SingleImageAquisition(1, cameraRecorderPointer, cameraPointer, NULL, NULL, NULL, TotalLine, &imgWidth, &imgHeight);
