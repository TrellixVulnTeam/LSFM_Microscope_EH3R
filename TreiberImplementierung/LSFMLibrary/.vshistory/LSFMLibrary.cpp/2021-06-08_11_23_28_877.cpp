...
bool SingleImageAquisition(int cameratyp, char* filename)
{
   ...
   //vector<WORD*>bufferList = flimCameracontroller->Live_View(2, camHandle, camRecorderHandle, &imgWidth, &imgHeight);
	...
	if (cameratyp == 1)
	{
		result = pmicIm_1->SingleImageAquisition(1, cameraRecorderPointer, cameraPointer, NULL, NULL, NULL, filename);

	}
	else
	{

		result = pmicIm_2->SingleImageAquisition(2, flimCameraRecorderPointer, flimCameraPointer, PhaseNumber::Four, PhaseSymmetry::twice, TapSelect::both, filename);
...
void  GetExposureTime(int camera, int* exposureTime, int* delay)
{
	...
	{
		WORD dwexposureTime, dw_delay;
		pmicIm_2->GetExposureAndDelayTime(flimCameraPointer, &dwexposureTime, &dw_delay);
...
void SetExposureTime(int camera, int exposureTime, int delay)
{
	...
	else
	{
		DWORD dwExpousreTime = DWORD(exposureTime);
		DWORD dwTimebase = DWORD(delay);
		pmicIm_2->SetExposureTime(flimCameraPointer, exposureTime, delay);
	}
}
...
DataPack* CameraLiveView()
{
	...
	return pmicIm_1->Live_View(1, cameraPointer, cameraRecorderPointer);
		
}

bool SetFlimParamter(int phaseNumber, int frequency)
{
	cout << frequency << endl;
	PhaseNumber phaseNumber1 = static_cast<PhaseNumber>(phaseNumber);
	pmicIm_2->SetFilmPhaseSequenceParameter(phaseNumber1, PhaseSymmetry::twice, PhaseOrder::opposite, TapSelect::both);
...
{
	return pmicIm_2->Live_View(2, flimCameraPointer, flimCameraRecorderPointer);
}

void Image_Sequence(int cameraType, const char* path, double zUp, double zDown, double xValue, double yValue, double focusUp, double focusDown, int N, double dz)
{
	...
	else
	{
		for (int i = 0; i < N; i++)
		{
			...
			pmicIm_2->SingleImageAquisition(2, flimCameraRecorderPointer, flimCameraPointer, PhaseNumber::Four, PhaseSymmetry::twice, TapSelect::both, TotalLine);
