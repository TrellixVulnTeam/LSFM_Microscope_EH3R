...
DataPack* CameraLiveView()
{
	...
	vector<WORD*>bufferList = pmicIm_1->Live_View(1, cameraPointer, cameraRecorderPointer, &imgWidth, &imgHeight);

	datapack->width = imgWidth;
	datapack->height = imgHeight;
	datapack->buffersize = (__int64)imgWidth * (__int64)imgHeight;
	if (bufferList.size() > 0)
		datapack->bufferptr = bufferList.at(0);


		return datapack;
}

DataPack_FLIM* FLimCameraLiveView()
{

	int imgWidth, imgHeight;
	DataPack_FLIM* datapack = new DataPack_FLIM();
	vector<WORD*>bufferList = pmicIm_2->Live_View(2, flimCameraRecorderPointer, flimCameraPointer, &imgWidth, &imgHeight);
