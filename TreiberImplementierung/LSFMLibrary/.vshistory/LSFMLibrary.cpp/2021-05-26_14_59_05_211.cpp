...
DataPack_FLIM* SingleImageAquisition(int cameratyp, char* filename)
{
	...
	if (cameratyp == 1)
	{
		vector<WORD*> bufferList = pmicIm_1->SingleImageAquisition(1, cameraRecorderPointer, cameraPointer, NULL, NULL, NULL, filename, &imgwidth, &imgheight);

	}
	else
	{

		vector<WORD*> bufferList = pmicIm_2->SingleImageAquisition(2, flimCameraRecorderPointer, flimCameraPointer, PhaseNumber::Four, PhaseSymmetry::twice, TapSelect::both, filename, &imgwidth, &imgheight);
