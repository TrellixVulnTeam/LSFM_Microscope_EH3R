...
vector<WORD*> LSFM_Camera::GetAndSave_Image(CameraTyp cameraTyp, HANDLE recorder, HANDLE camera, PhaseNumber phaseNumber, PhaseSymmetry phasesymmetry, TapSelect tapSelect, char* file_name, int*img_Width, int*img_Height)
{
...
vector<WORD*> LSFM_Camera::SingleImageAquisition(int cameraTyp, HANDLE recorder, HANDLE camera, int phaseNumber, int phasesymmetry, int tapSelect, char* file_name, int* imgWidth, int* imgHeight)
{
	...
	if (IsNoErrorOccured(iRet, "Eror during the Camera Busy Status acquisiton"))
	{
		...
		else
		{
			...
			if (IsNoErrorOccured(iRet, "Error during the Trigger Command"))
			{
				...
				vector<WORD*> bufferList = GetAndSave_Image(cameratyp_i, recorder, camera, phaseNumber_i, PhaseSymmetry_i, tapSelect_i, file_name, imgWidth, imgHeight);
