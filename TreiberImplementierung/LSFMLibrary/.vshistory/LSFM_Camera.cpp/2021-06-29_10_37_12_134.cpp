...
{
	cout << "Test Camera connection" <<  this->isNoCameraConnected << endl;
...
void LSFM_Camera::Disconnect()
{
	...
	//Stop Recorder
	iRet = PCO_RecorderStopRecord(this->hRec, this->cam);

	//Delete Recorder
	iRet = PCO_RecorderDelete(this->hRec);
...
DataPack* LSFM_Camera::Live_View()
{
	...
	else
	{
		...
		iRet = PCO_RecorderGetSettings(this->hRec, this->cam, NULL, NULL, NULL, &imgWidth, &imgHeight, NULL);
						...
						if (IsNoErrorOccured(iRet, "Error during the Trigger Command"))
						{
							...
							if (IsNoErrorOccured(iRet, "Error while reading FLIM parameters"))
							{
								...
								//file_name = strcpy(file_name, "C:\\\\LSFM_Software\\\\LSFM_Interface\\\\LSFM_Interface\\\\tmp");
									...
									iRet = PCO_RecorderCopyImage(this->hRec, this->cam, 1, 1, 1, this->imgWidth, this->imgHeight, imgBuffer, NULL, &metadata, NULL);
