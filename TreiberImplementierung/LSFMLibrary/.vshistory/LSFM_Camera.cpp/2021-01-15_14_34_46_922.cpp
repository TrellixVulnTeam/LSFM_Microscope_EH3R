...
#include "include/file12.cpp"
	...
	bool LSFM_Camera::GetAndSave_Image(CameraTyp cameraTyp, HANDLE camera, PhaseNumber phaseNumber, PhaseSymmetry phasesymmetry, TapSelect tapSelect, char* file_name)
	{
		...
		else if (cameraTyp == 1)
		{
			...
			bufsize = XResAct * YResAct * L * sizeof(WORD);
	...
	void LSFM_Camera::SingleImageAquisition(CameraTyp cameraTyp, HANDLE camera, PhaseNumber phaseNumber, PhaseSymmetry phasesymmetry, TapSelect* tapSelect, char* file_name)
	{
		...
		if (IsNoErrorOccured(iRet, "Eror during the Camera Busy Status acquisiton"))
		{
			...
			else
			{
				...
				if (IsNoErrorOccured(iRet, "Error during the Trigger Command"))
				{
					...
					GetAndSave_Image(cameraTyp, camera, phaseNumber, phasesymmetry, tapSelect,  file_name);
