					...
					if (IsNoErrorOccured(iRet, "Error during the Trigger Command"))
					{
						...
						else if (cameraTyp == 2)//PCO FLIM
						{
							...
							if (IsNoErrorOccured(iRet, "Error with the Acquisition of the flim Parameters"))
							{
								...
								WORD* imgBuffer1 = new WORD[(__int64)1008 * (__int64)1008];
								WORD* imgBuffer2 = new WORD[(__int64)1008 * (__int64)1008];
								WORD* imgBuffer3 = new WORD[(__int64)1008 * (__int64)1008];
								WORD* imgBuffer4 = new WORD[(__int64)1008 * (__int64)1008];

								Sleep(10);

								iRet = PCO_RecorderCopyImage(recorder, hCamArr[0], 0, 1, 1, 1008, 1008, imgBuffer1, NULL, &metadata, NULL);
								datapack->bufferptr1 = imgBuffer1;

								iRet = PCO_RecorderCopyImage(recorder, hCamArr[0], 1, 1, 1, 1008, 1008, imgBuffer2, NULL, &metadata, NULL);
								datapack->bufferptr2 = imgBuffer2;

								iRet = PCO_RecorderCopyImage(recorder, hCamArr[0], 2, 1, 1, 1008, 1008, imgBuffer3, NULL, &metadata, NULL);
								datapack->bufferptr3 = imgBuffer3;

								iRet = PCO_RecorderCopyImage(recorder, hCamArr[0], 3, 1, 1, 1008, 1008, imgBuffer4, NULL, &metadata, NULL);
