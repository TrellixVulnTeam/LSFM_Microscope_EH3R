		...
		for (int i = 0; i < controlerList.size(); i++)
		{
			...
			for (int j = 0; j < list1.size(); j++)
			{
				...
				if (s1.find(s2) != std::string::npos) {
					...
					cout << "Config stage " << stage << endl;
					LSFM_Motion_PI motion(char_array, stage);
					cout << "Motion" << motion.GetStage() << endl;
...
vector<LSFM_Motion_PI> controllerDetection(char* description, int descriptionSize)
{
	...
	*/
	...
	const char* fileName = "C:\\Users\\admin\\Documents\\Visual Studio 2019\\lsm_test\\TreiberImplementierung\\LSFMLibrary\\ConfigFile.txt";
